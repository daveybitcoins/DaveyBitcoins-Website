...
const HISTORICAL_RISK_BAND_DAYS = Math.round(4 * FAIR_VALUE_DAYS_PER_YEAR);
...
async function main() {
  ...
  function renderPriceChart() {
    ...
    // Extend the same threshold curves into the trailing four-year momentum
    // window so each forecast boundary visibly continues its own history.
    if(fullProjectionView) {
      const historicalBandStartMs=lastDateMs-HISTORICAL_RISK_BAND_DAYS*864e5;
      PROJECTED_RISK_BOUNDARIES.forEach(risk=>{
        ctx.strokeStyle=risk===0.50?'#58c56f':riskColor(risk,0.72);
        ctx.lineWidth=risk===0.50?1.8:1.2;ctx.setLineDash(risk===0.50?[10,5]:[4,5]);ctx.beginPath();
        let started=false;
        for(let i=s;i<=e;i+=S){
          const pointMs=dateMs(pts[i].date);
          if(pointMs<historicalBandStartMs) continue;
          const x=xOf(i),y=yOf(priceAtRiskForPoint(pts[i],risk));
          started?ctx.lineTo(x,y):ctx.moveTo(x,y);started=true;
        }
        const finalY=yOf(priceAtRiskForPoint(last,risk));
